...
export default function App() {
  ...
  const handleTraceComplete = (result: TraceResult) => {
    ...
    const summaryText = result.summary?.text ?? result.explanation ?? 'Summary unavailable';
  ...
  // Handle user clicking "View Graph" on a past history item
    ...
    if (item.raw) {
      setActiveTrace(item.raw);
      setCurrentTab('new-trace');
      window.scrollTo({ top: 300, behavior: 'smooth' });
    }
    // Seeded/mock history items (no `raw`) have no live data to reopen.
  };
