...
export const NodeDossierModal: React.FC<NodeDossierModalProps> = ({ node, onClose }) => {
  ...
  const riskBadge = getRiskBadge(node.riskLevel ?? 'unassessed');
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className={`p-3.5 rounded-xl border flex items-center justify-between ${riskBadge.bg}`}>
            ...
            <span className="font-mono text-[11px]">
              VASP MATCH: {node.vaspMatch ? 'YES' : 'NO'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <span className="text-[10px] font-mono text-[#c2c6d7]">
                {node.amountUsd != null ? `≈ $${node.amountUsd.toLocaleString()}` : 'USD N/A'}
              </span>
            </div>
            ...
            <div className="p-3 bg-[#1a1b21] rounded-xl border border-[#292a2f]">
              ...
              <span className="font-mono font-bold text-sm text-[#9ccaff] block mt-0.5">
                {node.blockNumber != null ? `#${node.blockNumber}` : 'N/A'}
              </span>
              <span className="text-[10px] font-mono text-[#c2c6d7]">
                {node.timestamp ?? 'N/A'}
              </span>
            ...
            <div className="p-3 bg-[#1a1b21] rounded-xl border border-[#292a2f]">
              ...
              <span className="font-mono font-bold text-sm text-[#e3e1e9] block mt-0.5">
                {node.gasUsedGwei != null ? `${node.gasUsedGwei} Gwei` : 'N/A'}
              </span>
              <span className="text-[10px] font-mono text-[#10b981]">
                {node.gasUsedGwei != null ? 'Optimal Execution' : 'Unavailable'}
              </span>
            </div>

            <div className="p-3 bg-[#1a1b21] rounded-xl border border-[#292a2f]">
              <span className="text-[10px] font-mono uppercase text-[#c2c6d7] block">Estimated Share</span>
              <span className="font-mono font-bold text-sm text-[#ff6b00] block mt-0.5">
                {node.details.flowPct != null ? `${node.details.flowPct.toFixed(1)}%` : 'N/A'}
              </span>
              <span className="text-[10px] font-mono text-[#c2c6d7]">
                {node.details.flowPct != null ? 'Heuristic allocation' : ''}
              </span>
            </div>
          </div>
