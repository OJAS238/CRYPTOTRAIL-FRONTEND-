...
export const ApiDocsModal: React.FC<ApiDocsModalProps> = ({ isOpen, onClose }) => {
  ...
  const curlExample = `curl -X POST "http://localhost:3001/api/cryptotrail/trace" \\
  -H "Authorization: Bearer NOT_REQUIRED_LOCALLY" \\
...
client = requests.post(
    "http://localhost:3001/api/cryptotrail/trace",
    headers={"Authorization": "Bearer NOT_REQUIRED_LOCALLY"},
    json={
        "address": "0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
        "chain": "ethereum",
        "max_depth_hops": 4,
...
print("Attributed CEX:", trace_data["attributionExchange"])
print("Confidence Score:", trace_data["confidenceScore"])`;

  const nodeExample = `const response = await fetch('http://localhost:3001/api/cryptotrail/trace', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ address: '0x...', chain: 'ethereum' })
});
const result = await response.json();
console.log(result.attributionExchange, result.explanation);`;
  return (
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-[#1a1b21] rounded-xl border border-[#292a2f] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono text-xs text-[#e3e1e9]">
                    Not implemented: /v2/exchange/attributions/{'{cluster_id}'}
                  </span>
                </div>
