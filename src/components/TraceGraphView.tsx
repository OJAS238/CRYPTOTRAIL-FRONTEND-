...
export const TraceGraphView: React.FC<TraceGraphViewProps> = ({
  ...
  const [isExporting, setIsExporting] = useState(false);
    ...
    const copySummary = () => {
    const usdText = trace.volumeTracedUsd != null ? `$${trace.volumeTracedUsd.toLocaleString()}` : 'N/A';
    const summary = `CryptoTrail Forensic Report
Target: ${trace.queryAddress}
Chain: ${trace.chain}
Destination Attributed: ${trace.attributionExchange}
Attribution Score: ${trace.confidenceScore}/100
Total Hops: ${trace.totalHops}
Mixer Flagged: ${trace.flaggedMixers ? 'YES (High Risk)' : 'Not assessed'}
Volume: ${trace.volumeTracedEth} ETH (${usdText})
Timestamp: ${trace.timestamp}`;
  ...
  return (
    <div className="w-full mt-6 space-y-6 animate-in fade-in duration-200">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-[#ff6b00]/20 text-[#ffb693] border border-[#ff6b00]/30">
                {trace.confidenceScore}/100 SCORE
              </span>
              {trace.flaggedMixers && (
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs font-mono text-[#c2c6d7]">
              <span>Matched Address: <span className="text-[#e3e1e9]">{trace.receivingHotWallet.substring(0, 6)}...{trace.receivingHotWallet.substring(trace.receivingHotWallet.length - 4)}</span></span>
              {trace.depositMemo && (
      ...
      <div className="w-full p-5 bg-[#1a1b21] border border-[#292a2f] rounded-2xl space-y-3">
        {trace.demo && <p className="text-xs text-[#ffb693]">Illustrative demo — not verified on-chain activity.</p>}
        <h3 className="text-base font-semibold text-[#ffb693]">{trace.summary?.source === 'ai' ? 'AI Summary' : 'Trace Summary'}</h3>
        <p className="text-sm text-[#c2c6d7] leading-relaxed">{trace.summary?.text ?? trace.explanation ?? 'Summary unavailable'}</p>
        {trace.summary?.reason && <p className="text-xs text-[#c2c6d7]">{trace.summary.reason}</p>}
        {trace.rawTrace?.candidates[0] && <details><summary className="cursor-pointer text-xs text-[#ffb693]">(?) Score details</summary><p className="text-xs mt-2">{trace.rawTrace.candidates[0].scoreBreakdown.hopScore} path length + {trace.rawTrace.candidates[0].scoreBreakdown.valueScore} value share + {trace.rawTrace.candidates[0].scoreBreakdown.matchScore} match</p></details>}
        <details><summary className="cursor-pointer text-xs text-[#ffb693]">Learn more about this estimate</summary><p className="text-xs mt-2 leading-relaxed">Wallets are expanded once; converging paths may be undercounted and same-second transfers are excluded. Scores are heuristic estimates, not proof of ownership. This diagram shows one connected path; the exported report contains the full graph.</p>{trace.warnings?.map((warning,i)=><p className="text-xs mt-2" key={i}>{warning}</p>)}</details>
      </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-mono text-xs uppercase tracking-widest text-[#c2c6d7]">
              Deterministic Traversal Flowchart ({trace.totalHops} Hops Identified)
            </h3>
          </div>
              ...
              <React.Fragment key={node.id}>
                ...
                >
                  ...
                  <div className="mt-3 pt-2 border-t border-[#292a2f]/60 flex items-center justify-between text-[11px] font-mono">
                    ...
                    <span className="text-[#ffb693] font-bold">{node.amountEth.toLocaleString(undefined, { maximumFractionDigits: 6 })} ETH</span>
                  </div>
      ...
      <div className="p-4 bg-[#0d0e13] rounded-2xl border border-[#1e1f25]">
        <div className="flex items-center justify-between mb-3 text-xs font-mono text-[#c2c6d7]">
          ...
          <span>TOTAL REQUEST TIME: {trace.avgLatencyMs}ms</span>
        </div>
