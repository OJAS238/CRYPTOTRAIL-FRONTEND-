...
export const NewTraceView: React.FC<NewTraceViewProps> = ({
  ...
  const clearInput = () => {
    ...
    const handleStartTrace = async (addressOverride?: string, chainOverride?: SupportedChain, demo = false) => {
    if (isScanning) return;
    const targetAddr = (addressOverride || inputValue).trim();
    const targetChain = chainOverride || selectedChain;

    if (!/^0x[a-fA-F0-9]{40}$/.test(targetAddr)) {
      setValidationError('Enter a valid Ethereum wallet address.');
      return;
    }
    if (targetChain !== 'Ethereum') {
      setValidationError('Only Ethereum tracing is currently supported by the backend.');
      return;
    }
    ...
    const t1 = setTimeout(() => setScanStep(2), 0);
    const t2 = setTimeout(() => setScanStep(2), 0);

    try {
      const response = await fetch('/api/cryptotrail/trace', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(demo ? { demo: 'multihop', chain: 'ethereum' } : { address: targetAddr, chain: 'ethereum' }),
      ...
      if (!response.ok) {
        setValidationError(data.error ?? 'Trace failed.');
        return;
      }
      ...
      clearTimeout(t1);
      clearTimeout(t2);
      setValidationError('Could not reach the tracing backend. Is it running on http://localhost:3001?');
    } finally {
      setIsScanning(false);
      setScanStep(0);
    }
      ...
      <div className="relative w-full max-w-[760px] flex flex-col items-center">
        ...
        <div className="flex flex-col items-center text-center w-full z-10">
          ...
          <p className="mt-4 text-base text-[#c2c6d7] max-w-[560px] leading-relaxed">
            Trace outgoing Ethereum transfers across up to four hops and review evidence-based exchange matches.
          </p>
        </div>
        ...
        <div className="w-full mt-8 z-20">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 text-xs">
            ...
            {PRESETS.map(preset => (
              <button
                key={preset.id} disabled={isScanning}
                type="button"
                onClick={() => {
                  applyPreset(preset);
                  handleStartTrace(preset.address, preset.chain, preset.id === 'multihop');
                }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mt-12 z-10">
          ...
          <div className="flex flex-col p-5 bg-[#1e1f25] border border-[#292a2f] rounded-xl shadow-sm hover:bg-[#292a2f] transition-all">
            ...
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-2xl font-bold text-[#e3e1e9]">{activeTrace ? (activeTrace.avgLatencyMs / 1000).toFixed(2) + 's' : '—'}</span>
              <span className="font-mono text-xs text-[#9ccaff]">REQUEST TIME</span>
            </div>
            <div className="mt-2 flex items-center">
              <div className="h-1 w-full bg-[#121318] rounded-full overflow-hidden flex">
            ...
            <p className="mt-3 text-xs text-[#c2c6d7] leading-relaxed">
              Measured duration of the completed backend request.
            </p>
          </div>
          ...
          <div className="flex flex-col p-5 bg-[#1e1f25] border border-[#292a2f] rounded-xl shadow-sm hover:bg-[#292a2f] transition-all">
            ...
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-2xl font-bold text-[#e3e1e9]">{activeTrace ? activeTrace.confidenceScore + '/100' : '—'}</span>
              <span className="font-mono text-xs text-[#c2c6d7]">HEURISTIC SCORE</span>
            </div>
            <div className="mt-2 flex items-center">
              <div className="h-1 w-full bg-[#121318] rounded-full overflow-hidden flex">
                <div className="h-full bg-[#ffb693] w-full"></div>
              </div>
            </div>
            <p className="mt-3 text-xs text-[#c2c6d7] leading-relaxed">
              Heuristic match score, not a probability or certainty.
            </p>
          </div>
        </div>
        ...
        <div className="mt-12 pt-4 flex flex-wrap items-center justify-between gap-4 w-full border-t border-[#1e1f25]/50">
          <div className="flex items-center gap-3 text-xs font-mono text-[#c2c6d7]">
            <span>{activeTrace?.demo ? 'SOURCE: ILLUSTRATIVE FIXTURE' : 'SOURCE: ETHERSCAN'}</span>
            <span>·</span>
